...
/*   Updated: 2023/06/05 18:49:54 by jaehjoo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	show_player(t_img *img, int box, t_vec player)
{
	...
	minimap_put_pixel_to_img(img, cur.x, cur.y, 0xff6347);
	minimap_put_pixel_to_img(img, cur.x + 1, cur.y, 0xff6347);
	minimap_put_pixel_to_img(img, cur.x, cur.y + 1, 0xff6347);
	minimap_put_pixel_to_img(img, cur.x - 1, cur.y, 0xff6347);
	minimap_put_pixel_to_img(img, cur.x, cur.y - 1, 0xff6347);
}
