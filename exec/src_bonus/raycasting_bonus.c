...
/*   Updated: 2023/06/07 16:41:17 by jaehjoo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_delta_dist(t_total *app, char flag)
{
	if (flag == 'x')
	{
		if (app->ray.ray_dir.y == 0)
			app->ray.delta_dist.x = 0;
		else if (app->ray.ray_dir.x == 0)
			app->ray.delta_dist.x = 1;
		else
			app->ray.delta_dist.x = fabs(1 / app->ray.ray_dir.x);
	}
	else
	{
		if (app->ray.ray_dir.x == 0)
			app->ray.delta_dist.y = 0;
		else if (app->ray.ray_dir.y == 0)
			app->ray.delta_dist.y = 1;
		else
...
void	raycasting(t_total *app)
{
	...
	while (x < app->win.x)
	{
		...
		set_delta_dist(app, 'x');
		set_delta_dist(app, 'y');
		app->ray.hit = 0;
		find_step(&app->ray);
		wall_hit(app);
		draw_texture(app, x);
